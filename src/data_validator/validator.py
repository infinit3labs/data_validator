...
class DataValidator:
    ...
    def validate_configured_tables(self) -> Dict[str, ValidationSummary]:
        """Validate all tables using data sources defined in the configuration."""
        data_sources = {}
        missing_sources = []

        for table in self.config.tables:
            if table.data_source is not None:
                data_sources[table.name] = table.data_source
            else:
                missing_sources.append(table.name)

        if missing_sources:
            raise ValueError(
                "Data source missing for tables: " + ", ".join(missing_sources)
            )

        return self.validate_all_tables(data_sources)
